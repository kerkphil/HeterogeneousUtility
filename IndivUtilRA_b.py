...
def mdefs(Kp, Bp, K, B, LS, LU, z, params):
    ...
    if lam==1:
        ...
        USD = np.log(CSD) + AS*(LS**(1-lam)-1)/(1-lam)
        UUD = np.log(CUD) + AU*(LU**(1-lam)-1)/(1-lam)
        USI = np.log(CSI) + AS*(LS**(1-lam)-1)/(1-lam)
        UUI = np.log(CUI) + AU*(LU**(1-lam)-1)/(1-lam)     
...
def mdyn(theta, params):
    ...
    E3 = (CS)**(-1)*wS - AS*LS**(-lam)   # (C)**(-1)*wS - AS*LS**(-1)
    E4 = (CU)**(-1)*wU - AU*LU**(-lam)   # (C)**(-1)*wU - AU*LU**(-1)
